...
def registration():
    if request.method == 'POST':
        ...
        if password == '' or login == '' or name == '':
            return render_template('registration.html', Flag_4=True)
        cursor.execute("SELECT * FROM service.users WHERE login='%s'" % (login))
        records_2 = list(cursor.fetchall())
        if records_2:
            return render_template('registration.html', Flag_2=True)
        else:
